...
parser.add_argument('-a', '--alarm_min_SpO2', type=int, help='SpO2 minimal level. Default = 92', default=92, required=False)
parser.add_argument('-d', '--device', type=str, help='path to CMS device. Example: /dev/ttyUSB0 (default)', default='/dev/ttyUSB0', required=False)
parser.add_argument('-o', '--output', action='store_true', default=False)
parser.add_argument('-m','--macAddress', type=str, help='(bluetooth) mac Address of vibrator smartphone. Example E0:48:D3:C2:39:83. Default None', required=False, default=None)
...
def handle_press(event):
    global vib
    print('press', event.key); sys.stdout.flush()
...
def check_cms_disconnection():
    ...
    elif check_cms_disconnection_count < 5: return # wait 5 * 2 seconds
    else:
        if not cms.beat.searching: vib.vibrate(vib.signal.disconnected)
    return True
...
def alarm():
    ...
    if not check_cms_disconnection_count:
        ...
        vibrate = min([vibrate, 1900])
        vibrate = max([vibrate, 0])
        if vibrate > 0: vibrating = True
        else: vibrating = False
        s="%d,0,0,0,0" % vibrate
        # print(s)
...
def proc_key(key):
    ...
    if key == " " or \
            (vib.veto and alarm_flip_time and
             ((time.time() - alarm_flip_time) > 15*60)
            ):
...
vib.verbose = 1
vib._what_services()
vib.veto = True ; alarm_flip_time = None # start with alarm off. space key is needed to start
...
while True:
    ...
    time.sleep(max([2-(time.time()-now), 0.001]))
